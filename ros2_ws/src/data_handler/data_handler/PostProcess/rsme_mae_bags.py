...
#         if topic == gt_topic:
#             msg = deserialize_message(data, TwistStamped)
#             gt_t.append(ts); gt_v.append(msg.twist.linear.x)
#         elif topic == meas_topic:
#             msg = deserialize_message(data, meas_type)
#             m_t.append(ts); m_v.append(get_field(msg, field_path))
#     return (np.array(gt_t), np.array(gt_v)), (np.array(m_t), np.array(m_v))
...
# def find_optimal_shift(gt_grid, m_grid, dt):
#     corr = correlate(gt_grid - gt_grid.mean(),
#                      m_grid  - m_grid.mean(),
#                      mode='full')
#     lags  = np.arange(-len(m_grid)+1, len(gt_grid))
#     idx   = np.argmax(corr)
...
# def main():
#     runs = [
#         ("run1", "/home/docker/OpticalFlowSlider/ros2_ws/my_rosbag/Experiment2/LFN3/Combined/LFN3_smooth_KF_640/LFN3_smooth_KF_640_0.db3"),
#         ("run2", "/home/docker/OpticalFlowSlider/ros2_ws/my_rosbag/Experiment2/LFN3/Combined/LFN3_smooth_KF_640_1/LFN3_smooth_KF_640_1_0.db3"),
#         ("run3", "/home/docker/OpticalFlowSlider/ros2_ws/my_rosbag/Experiment2/LFN3/Combined/LFN3_smooth_KF_640_2/LFN3_smooth_KF_640_2_0.db3"),
#         ("run4", "/home/docker/OpticalFlowSlider/ros2_ws/my_rosbag/Experiment2/LFN3/Combined/LFN3_smooth_KF_640_3/LFN3_smooth_KF_640_3_0.db3"),
#         ("run5", "/home/docker/OpticalFlowSlider/ros2_ws/my_rosbag/Experiment2/LFN3/Combined/LFN3_smooth_KF_640_4/LFN3_smooth_KF_640_4_0.db3"),
#     ]
#     gt_topic = "/motor/present_velocity"
#     measurement_topics = [
#         ("IMU (filtered)",  "/kalman_filter/imu_filtered",       Float64,        "data",     False),
...
#     # prepare per‐topic collector
#     per_topic_metrics = {
#         name: {"rmse":[], "mae":[], "rmse_shift":[], "mae_shift":[], "taus":[]} 
#         for name, *_ in measurement_topics
#     }

#     # open per‐run CSV
#     run_csv = "metrics_summary_IMU_ONLY.csv"
#     with open(run_csv, "w", newline="") as cf:
#         run_writer = csv.writer(cf)
#         run_writer.writerow([
#             "run","topic",
#             "rmse","mae",
#             "shift_s","rmse_shift","mae_shift"
#         ])

#         # print header
#         hdr = f"{'run':<6} | {'topic':<15} | {'RMSE':>6} | {'MAE':>6} | {'shift(s)':>8} | {'RMSE_shift':>10} | {'MAE_shift':>9}"
#         sep = "-"*len(hdr)
#         print(hdr)
#         print(sep)
...
#                 print(f"  missing bag: {bag_path}")
#                 continue

#             for name, topic, msg_cls, field, needs_shift in measurement_topics:
#                 try:
#                     (gt_t, gt_v),(m_t, m_v) = extract_velocity_data(
#                         bag_path, gt_topic, topic, msg_cls, field
#                     )
#                 except Exception:
#                     print(f"  {name:<15} extract error")
#                     continue

#                 if gt_t.size<2 or m_t.size<2:
#                     print(f"  {name:<15} insufficient data")
#                     continue

#                 t0, t1 = max(gt_t[0], m_t[0]), min(gt_t[-1], m_t[-1])
#                 if t1<=t0:
#                     print(f"  {name:<15} no overlap")
#                     continue

#                 t_grid  = np.arange(t0, t1, dt)
#                 gt_grid = interpolate_to_grid(gt_t, gt_v, t_grid)
#                 m_grid  = interpolate_to_grid(m_t, m_v, t_grid)

#                 rmse_ns = compute_rmse(gt_grid, m_grid)
#                 mae_ns  = compute_mae (gt_grid, m_grid)
#                 shift_s = ""
#                 rmse_s  = ""
#                 mae_s   = ""

#                 per_topic_metrics[name]["rmse"].append(rmse_ns)
#                 per_topic_metrics[name]["mae"].append(mae_ns)

#                 if needs_shift:
#                     steps, tau = find_optimal_shift(gt_grid, m_grid, dt)
#                     shift_s = f"{tau:+.3f}"
#                     if steps>0:
#                         m_al, g_al = m_grid[:-steps], gt_grid[steps:]
#                     else:
#                         m_al, g_al = m_grid[-steps:], gt_grid[:steps]
#                     rmse_s_val = compute_rmse(g_al, m_al)
#                     mae_s_val  = compute_mae (g_al, m_al)
#                     rmse_s = f"{rmse_s_val:.4f}"
#                     mae_s  = f"{mae_s_val :.4f}"
#                     per_topic_metrics[name]["taus"].append(tau)
#                     per_topic_metrics[name]["rmse_shift"].append(rmse_s_val)
#                     per_topic_metrics[name]["mae_shift"].append(mae_s_val)

#                 print(f"{run_label:<6} | {name:<15} | "
#                       f"{rmse_ns:6.4f} | {mae_ns:6.4f} | "
#                       f"{shift_s:>8} | {rmse_s:>10} | {mae_s:>9}")

#                 run_writer.writerow([
#                     run_label,name,
#                     f"{rmse_ns:.4f}",f"{mae_ns:.4f}",
#                     shift_s,rmse_s,mae_s
#                 ])

#     # now compute and save averages
#     avg_csv = "metrics_average.csv"
#     with open(avg_csv, "w", newline="") as af:
#         avg_writer = csv.writer(af)
#         avg_writer.writerow([
#             "topic","avg_rmse","avg_mae",
#             "avg_shift","avg_rmse_shift","avg_mae_shift"
#         ])

#         print("\n=== AVERAGE OVER ALL RUNS ===")
#         for name, m in per_topic_metrics.items():
#             avg_rmse = np.mean(m["rmse"]) if m["rmse"] else np.nan
#             avg_mae  = np.mean(m["mae"] ) if m["mae"]  else np.nan

#             row = [name, f"{avg_rmse:.4f}", f"{avg_mae:.4f}"]

#             if m["taus"]:
#                 avg_tau = np.mean(m["taus"])
#                 avg_rs  = np.mean(m["rmse_shift"])
#                 avg_ms  = np.mean(m["mae_shift"])
#                 row += [f"{avg_tau:+.3f}", f"{avg_rs:.4f}", f"{avg_ms:.4f}"]
#                 summary = (f"{name:<15} : RMSE={avg_rmse:.4f}, MAE={avg_mae:.4f}, "
#                            f"shift={avg_tau:+.3f}s, RMSE_shift={avg_rs:.4f}, MAE_shift={avg_ms:.4f}")
#             else:
#                 row += ["","", ""]
#                 summary = f"{name:<15} : RMSE={avg_rmse:.4f}, MAE={avg_mae:.4f}"
...
#     print(f"\n✅ per-run metrics → {run_csv}")
#     print(f"✅ average metrics → {avg_csv}")
